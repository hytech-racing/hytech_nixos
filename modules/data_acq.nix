  ...
  config = {
    ...
    systemd.services.data_writer = {
      ...
      serviceConfig.ExecStart =
        "${pkgs.py_data_acq_pkg}/bin/runner.py ${pkgs.proto_gen_pkg}/bin ${pkgs.ht_can_pkg} ${pkgs.ht_eth_protos_gen_pkg}/bin ${escapeShellArg cfg.mcu-ip} ${escapeShellArg cfg.send-to-mcu-port} ${escapeShellArg cfg.recv-from-mcu-port} ${escapeShellArg cfg.recv-ip}";
