        ...
        ExecStart =
          let
            script = pkgs.writeScript "configure-lte-modem" ''
              ...
              # Get the first device matching cdc-wdm[0-9]*
              # device="/dev/$DEVICE_NAME" # todo: get this from udev
              device=$(ls /dev/cdc-wdm[0-9]* 2>/dev/null | head -n 1)

              # Check if device is found
              if [ -n "$device" ]; then
                  echo "Device found: $device"
                  # Use $device variable for further operations
              else
                  echo "No device found matching pattern."
              fi
